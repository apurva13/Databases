...
-- To check if the Instructor teaches all 3 given subject in Courses table, I am using nested query to derive the  CourseName from Courses 
-- and counting these courses for each Instructor using Group BY.

SELECT Instructor FROM Course_Instructor 
WHERE CourseName IN (SELECT CourseName FROM Courses)
GROUP BY Instructor 
